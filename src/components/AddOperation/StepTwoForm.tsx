...
export const StepTwoForm = () => {
    ...
    useEffect(() => {
        if (chosenOperationType === "1" || chosenOperationType === "2") setAvailableAmount((actualPeriod as PeriodEntity).freeCashAmount);
        if (chosenOperationType === "9" && chosenOtherOperationType === "0") setAvailableAmount(null);
        if (chosenOperationType === "9" && chosenOtherOperationType === "3") setAvailableAmount((actualPeriod as PeriodEntity).savingsAmount);
    ...
    return <div className=" border-b-[1px] pb-8">
        <StepHeader text="Krok 2: Kwota"/>
        <div className="form-control w-fit max-w-xs block mx-auto">
